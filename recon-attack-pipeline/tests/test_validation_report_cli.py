...
class ValidationReportCliTests(unittest.TestCase):
    ...
    def test_validation_alias_status_uses_shared_database(self):
        ...
        with patch("aidast.validation.shared_validation_status", return_value=expected) as status:
            code, stdout, stderr = self.invoke(["validation", "status", "Pipeline.db"])
        self.assertEqual(code, 0, stderr)
        self.assertEqual(json.loads(stdout), expected)
        status.assert_called_once_with(Path("Pipeline.db"), scan_id=None, case_id=None)

    def test_legacy_validation_and_report_selectors_are_not_accepted(self):
        for arguments in (
            ["validate", "run", "Attack.db", "--run-id", "legacy"],
            ["report", "run", "Validation.db", "--platform", "hackerone",
             "--validation-id", "legacy"],
        ):
