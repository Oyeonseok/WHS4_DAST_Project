...
- 작성일: 2026-09-12
- 상태: HTTP/Playwright/OOB/Chain runtime 계약과 adapter 구현 완료
...
- 구현 여부: shared DB v9, DTO, 결정론적 판정, Repository, 복구, Report v2,
  ...
  native 단일 runner와 case별 tool-disabled Codex thread도 lazy 경로로 연결됐다.
...
| §6 Blind 경계 | DTO, 공개 필드, hash, tool-disabled 단일 native runner, case별 persisted thread 격리와 frozen assessment 재개 | 없음 |
| §7 shared DB | migration, repository, ownership, 실행 producer별 ledger 기록 및 observation request ID 소유권 검증 | 없음 |
| §8 판정 | evidence·점수·가설, profile-aware runtime 최소 proof 검사, target/negative proof 동등성, HTTP/DOM/OOB assertion, Playwright 및 configured HTTP OOB observer 실행 | 실제 target에서 fresh negative control이 clear인지 수용 검증 |
