...
## 2026-09-14: Recon 관측 전체 태깅 병렬화와 재개

변경 전에는 Katana 관측을 50건씩 하나의 Codex 호출로 순차 처리했다. 3,534건이면
71회의 모델 호출이 직렬 실행되고, 한 배치의 구조화 출력이 잘못되면 해당 관측 전체가
미태깅 상태로 남은 채 다음 배치로 넘어갔다.

변경 후에는 기본 100건 배치를 최대 3개까지 병렬 실행한다. 정제된 입력이 완전히 같은
관측은 한 번만 판정해 결과를 각 observation ID에 복제하고, 실패한 배치는 50건과
25건까지 나눠 재시도한다. Codex 로그인 확인도 Agent 인스턴스에서 한 번만 수행한다.
모델 호출 완료마다 전체 관측 기준 진행률과 재시도 대기 수를 출력한다.

`aidast annotations status/resume` 명령을 추가해 기존 shared `Pipeline.db`에서 완료된
annotation이 없는 관측만 조회하고 다시 태깅할 수 있게 했다. 모델 호출은 독립 임시
directory에서 수행하고 SQLite 결과 저장은 주 스레드에서 transaction으로 처리한다.
재시도 후에도 실패한 관측은 원본을 보존하며 resume 명령의 종료 코드와 요약에 남긴다.
