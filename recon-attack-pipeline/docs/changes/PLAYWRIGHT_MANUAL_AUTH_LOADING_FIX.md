...
> 2026-09-15 이후 수동 인증 POST의 권한 부여 방식은
> `MANUAL_AUTH_REQUEST_CAPABILITY.md`의 요청별 승인 방식으로 교체되었다. 이 문서의
> 브라우저 이벤트 펌프 수정은 그대로 유효하지만, 인증 단계 전체에 적용되는
> 임시 POST 권한은 더 이상 사용하지 않는다.
