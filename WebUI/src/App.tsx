...
import { stages, pipelineStageState, type Finding, type Snapshot } from './lib/events';
...
function Pipeline({ snapshot, language }: { snapshot: Snapshot; language: Language }) {
  return <ol className="pipeline" aria-label={translate(language, 'Scan workflow')}>{stages.map((s, i) => {
    const state = pipelineStageState(s, snapshot);
    const label = state === 'done' ? translate(language, 'Completed') : state === 'separate' ? translate(language, 'No report draft') : state === 'current' ? `${snapshot.progress}% · ${translate(language, snapshot.status)}` : translate(language, 'Pending');
    return <li key={s} className={state} aria-current={state === 'current' ? 'step' : undefined}><span className="stage-number">{state === 'done' ? <Icon name="check" size={13}/> : String(i + 1).padStart(2,'0')}</span><strong>{s}</strong><small>{label}</small></li>;
  })}</ol>;
}
...
export default function App() {
  ...
  const [modal, setModal] = useState<'new' | 'scope' | 'scope-workflow' | 'verified-scope' | 'finding' | 'report' | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      } catch (e) { if (!abort.signal.aborted) { setScopes([]); setLaunchError(e instanceof Error ? e.message : 'Approved scopes could not be loaded.'); } }
    })();
    return () => abort.abort();
  }, [modal, demo]);
  useEffect(() => {
  ...
  const scanPanel = <Panel className={page === 'Scans' ? 'scan-focus' : ''} title={demo ? tr('Local lab · API assessment') : snapshot?.program_name || scanId} subtitle={demo ? tr('Synthetic fixture · isolated from program inventory') : `${tr('Read-only scan snapshot')} · ${scanId}`} action={<Badge tone="success"><span className="dot"/>{tr(snapshot?.status || state)}</Badge>}>
    {snapshot ? <><Pipeline snapshot={snapshot} language={language}/><div className="scan-stats"><div><span>{tr('Scan ID')}</span><strong className="mono">{snapshot.scan_id}</strong></div><div><span>{tr('Endpoints')}</span><strong>{snapshot.endpoints}</strong></div><div><span>{tr('Request budget')}</span><strong>{snapshot.requests} <small>/ {snapshot.budget.toLocaleString()}</small></strong></div><div className="progress-stat"><span>{snapshot.stage} {tr('progress')} <b>{snapshot.progress}%</b></span><progress max="100" value={snapshot.progress} aria-label={`${snapshot.stage} ${tr('progress')}`}/>{snapshot.stage === 'Recon' && snapshot.status === 'running' && snapshot.activity && <p className="scan-current-activity"><span>{tr('Current work')}</span><strong>{tr(snapshot.activity)}</strong></p>}</div></div></> : <Empty title={tr(state === 'offline' ? 'Backend unavailable' : 'Loading scan snapshot')}>{tr('Connect the REST snapshot endpoint to display scan state. Demo data is never substituted in live mode.')}</Empty>}
  </Panel>;
  ...
  return <div className={`app-shell ${compact ? 'compact-mode' : ''}`}>
    ...
    <div className="main-shell"><header className="topbar"><div className="breadcrumb">{tr('Workspace')} <span>/</span> <strong>{pageLabel(page)}</strong></div><div className="top-actions"><span className={`connection ${demo ? 'demo' : ''}`}><span className="dot"/>{tr(demo ? 'Demo environment' : state)}</span><label className="language-select"><span className="sr-only">{tr('Language')}</span><select aria-label={tr('Language')} value={language} onChange={event => setLanguage(event.target.value as Language)}><option value="ko">한국어</option><option value="en">English</option></select></label><button className="icon-button theme-toggle" title={tr(`Switch to ${resolvedTheme === 'dark' ? 'light' : 'dark'} mode`)} aria-label={tr(`Switch to ${resolvedTheme === 'dark' ? 'light' : 'dark'} mode`)} onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')}><Icon name={resolvedTheme === 'dark' ? 'sun' : 'moon'} size={16}/></button></div></header>
      ...
      <div className={`workspace-grid ${collapsed ? 'activity-collapsed' : ''}`}><main id="main-content" tabIndex={-1} className="content-column">
        ...
        {page === 'Overview' && <div className="overview-layout">
        ...
        {page === 'Scans' && <>{!demo && <div className="toolbar"><label>{tr('Persisted scan')} <select aria-label={tr('Select persisted scan')} value={scanId} onChange={event => setScanId(event.target.value)}>{!scanOptions.some(item => item.scan_id === scanId) && scanId && <option value={scanId}>{scanId}</option>}{scanOptions.map(item => <option key={item.scan_id} value={item.scan_id}>{item.scan_id} · {tr(item.status)}</option>)}</select></label><button className="secondary-button" onClick={refresh}>{tr('Reload snapshot')}</button></div>}{scanPanel}</>}
