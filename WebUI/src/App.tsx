...
export default function App() {
  ...
  const scopeWorkflowContent = workflowProgram && <div className="scope-workflow">
    ...
    {scopeEvents.length > 0 && <div className="scope-event-list" aria-live="polite"><h3>{tr('Collection activity')}</h3>{scopeEvents.map(event => <div key={`${event.job_id}:${event.event_id}`} className={event.level}><time dateTime={event.occurred_at}>{new Date(event.occurred_at).toLocaleTimeString(language === 'ko' ? 'ko-KR' : 'en-GB', { hour12: false })}</time><span>{event.message}</span></div>)}</div>}
    {workflowProgram.scope_status === 'review_required' && scopeDraft && <>
      <div className="scope-review-heading"><div><span>{tr('UNAPPROVED DRAFT')}</span><h3>{scopeDraft.program_name}</h3><p>{scopeDraft.program_description}</p></div><code>{scopeDraft.scope_id}</code></div>
      <section className="scope-review-section"><h3>{tr('In scope')} <span>{scopeDraft.in_scope_assets.length}</span></h3><div className="scope-asset-list">{scopeDraft.in_scope_assets.map((asset, index) => <div key={`${asset.asset}:${index}`}><Badge tone="success">{asset.asset_type}</Badge><div><strong className="mono">{asset.asset}</strong><p>{asset.description || asset.eligibility || tr('No additional description.')}</p></div><small>{asset.maximum_severity || tr('Policy limit')}</small></div>)}</div></section>
      <section className="scope-review-section"><h3>{tr('Out of scope')} <span>{scopeDraft.out_of_scope_assets.length}</span></h3><div className="scope-asset-list">{scopeDraft.out_of_scope_assets.map((asset, index) => <div key={`${asset.asset}:${index}`}><Badge tone="critical">{asset.asset_type}</Badge><div><strong className="mono">{asset.asset}</strong><p>{asset.description || asset.eligibility || tr('Excluded by policy.')}</p></div></div>)}</div></section>
      <div className="scope-rule-grid"><section><h3>{tr('Allowed')}</h3><ul>{scopeDraft.allowed_activities.map((item, index) => <li key={index}>{item}</li>)}</ul></section><section><h3>{tr('Prohibited')}</h3><ul>{scopeDraft.prohibited_activities.map((item, index) => <li key={index}>{item}</li>)}</ul></section><section><h3>{tr('Operational constraints')}</h3><ul>{scopeDraft.operational_constraints.map((item, index) => <li key={index}>{item}</li>)}</ul></section><section><h3>{tr('Ambiguities to verify')}</h3><ul>{scopeDraft.ambiguities.length ? scopeDraft.ambiguities.map((item, index) => <li key={index}>{item}</li>) : <li>{tr('None extracted.')}</li>}</ul></section></div>
      <details className="scope-evidence"><summary>{tr('Source evidence and safe harbor')}</summary><p>{scopeDraft.safe_harbor || tr('No safe-harbor text was extracted.')}</p>{scopeDraft.source_evidence.map((item, index) => <blockquote key={index}><strong>{item.section}</strong>{item.quote}</blockquote>)}</details>
  ...
  return <div className={`app-shell ${compact ? 'compact-mode' : ''}`}>
    ...
    <dialog ref={dialog} onCancel={() => setModal(null)} onClose={() => setModal(null)} aria-labelledby="dialog-title">
      <div className="dialog-heading"><h2 id="dialog-title">{modal === 'new' ? tr('Start a new scan') : modal === 'scope' ? tr('Add bug bounty program') : modal === 'scope-workflow' ? tr('Collect and review Scope') : modal === 'report' ? tr(demo ? 'Demo report preview' : 'Local report preview') : selectedFinding?.id}</h2><button className="icon-button" aria-label={tr('Close dialog')} onClick={() => setModal(null)}>×</button></div>
