...
## 2026-09-15: AI-Dast 메인 + WHS 후속 파이프라인 병합

### 최종 소유권

`AI-Dast`를 유일한 실행 기준으로 유지한다. Scope, Auth, Recon 계획/실행,
Recon 도구, Recon 데이터 모델과 `Recon.db` 스키마는 AI-Dast 구현이 소유한다.
WHS 구현에서는 Native Attack, Chaining, Shared Validation, case 기반 Report와
그 전용 Skill만 후속 단계로 이식했다.

### AI-Dast Recon 및 DB에서 달라진 점

다음 Recon 구현은 교체하지 않았다.

- `src/aidast/recon/db.py`
- `src/aidast/recon/executor.py`
- `src/aidast/recon/surface.py`
- `src/aidast/orchestration/recon.py`
- `src/aidast/auth/**`
- `src/aidast/scope/**`

Wildcard 자산 배치, `AssetDiscovery.db`, 로그인 세션 분리,
runtime/system browser, `excluded_hosts`, deferred candidate, diagnostics,
`--tag-after`, Scope 수치 우선 정책은 그대로 유지된다.

변경된 Recon 인접 계약은 다음뿐이다.

| 항목 | 변경 내용 |
|---|---|
| TargetPolicy | Recon의 `allowed_methods`는 계속 안전 메서드 전용으로 유지하고, 별도 Attack/Validation 권한 필드를 추가 |
| 태깅 | deferred 태깅에서도 저장된 `endpoint_observations.evidence_json`을 다시 사용 |
| 통합 run | 미태깅·태깅 실패 관측이 남으면 Attack을 시작하지 않음 |
| Handoff | `Scope.md`를 해시된 handoff artifact에 추가 |
| DB 봉인 | `Recon.db`를 직접 수정하지 않고 검증된 SQLite backup으로 `Pipeline.db` 생성 |

`src/aidast/pipeline/materialize.py`가 Handoff artifact 해시를 검증하고
`Recon.db`를 `Pipeline.db`로 복제한다. 원본 SHA-256, 스키마와 user version은
변경하지 않는다. `src/aidast/pipeline/live_schema.py`의 v9 확장은 복제본에만
적용한다. 복제 출처는 update/delete가 금지된 `pipeline_sources`에 기록한다.

### WHS Attack에서 달라진 점

- Native Attack 모듈과 Hunt Skill 59개를 AI-Dast 패키지로 이식했다.
- 입력은 WHS Recon의 공유 DB가 아니라 AI-Dast Handoff에서 만든 `Pipeline.db`다.
- Skill 선택은 AI-Dast endpoint, parameter, observation, annotation과
  surface signal을 사용한다.
- 상태 변경 요청은 TargetPolicy 권한과 요청별 승인 envelope를 모두 요구한다.
- Native Agent는 `workspace-write` sandbox에서 실행하며 `Pipeline.db` 디렉터리를
  sandbox에 추가하지 않는다. Agent에는 `broker://pipeline` 토큰과 인증된 로컬
  helper client만 제공하고, sandbox 밖의 trusted broker가 allowlist된 DB/요청
  helper를 실행한다. 따라서 Shell로 DB를 직접 열거나 별도 네트워크 요청을 보낼
  수 없다.
- 완료 전에 finding, attempt, reproduction spec, 미해결 lead와 unknown request를
  DB와 다시 대조한다.
- 기존 AI-Dast 오프라인 `Attack.db` 경로는 legacy로 유지한다.

### WHS Chaining에서 달라진 점

- Validation 선행 계약 때문에 Native Attack 뒤에 항상 실행한다.
- 입증된 finding이 없으면 Agent를 호출하지 않고 `skipped`로 끝난다.
- 후보, 단계, 값 전달 binding과 terminal impact를 `Pipeline.db`에 기록한다.

### WHS Validation에서 달라진 점

- blind replay, positive/negative control, HTTP/browser/OOB/chain adapter,
  prerequisite development, DecisionEngine과 resume 상태 머신을 이식했다.
- Chaining이 `completed` 또는 `skipped`일 때만 시작한다.
- 실패 시 진행 중 작업을 `outcome_unknown`, case를 `interrupted`로 닫는다.
- 재개 시 같은 stage run과 case를 유지해 audit history를 보존한다.
- 기존 별도 `Validation.db` v1은 `aidast.validation.legacy`로 유지한다.

### WHS Report에서 달라진 점

- 새 경로는 `Pipeline.db`의 `validation_cases.case_id`를 입력으로 사용한다.
- `CONFIRMED`만 draft를 만들고 KNOWN은 원본 case, CONTESTED는 review-only로 처리한다.
- context, 인용 evidence와 Validation decision hash를 검증한다.
- decision hash가 바뀌면 기존 Report를 stale로 표시한다.
- 기존 `Validation.db` 기반 Report.db v1은 legacy API로 유지한다.

### 최종 CLI 흐름

```text
aidast run
  Scope → AI Recon → AI 태깅 → Handoff → Pipeline.db
  → WHS Native Attack → WHS Chaining → WHS Shared Validation

aidast report run Pipeline.db --case-id CASE --platform PLATFORM
```

`validate run/resume/status`는 shared Pipeline 입력을 지원하고,
기존 `--run-id`/`--validation-id` selector도 persisted legacy DB 호환을 위해
유지한다.
