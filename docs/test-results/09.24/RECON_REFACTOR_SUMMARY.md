...
최초 실행과 최종 실행을 비교하면 **실제 GET 요청과 최종 Surface 모두 정답 경로가 16개 증가**했다. 71개 대비 **12.7% → 35.2%, 약 22.5%p 상승, 2.78배**다. SPA 수정은 서버에 실제 요청해 확인한 경로를 늘렸고, 정규화 수정은 그중 최종 Surface에서 사라지던 경로를 보존했다. 정규화 수정 전후의 GET 요청 정답은 25개로 같고 Surface만 **9개 → 25개**로 변했다. [요청별 O/X](RECON_GET_ROUTE_MATRIX.md), [Surface별 O/X](RECON_SURFACE_ROUTE_MATRIX.md)에 71개 경로 전체가 표시돼 있다.
...
- 다음 평가 대상은 브라우저가 열지 않은 화면의 API 요청과 ID 값을 넣어야 하는 경로다. 큰 SecLists `common.txt`는 아직 실제 Recon에 사용하지 않았다.
