...
def _parser() -> argparse.ArgumentParser:
    ...
    run.add_argument(
        ...
        help="root for offline Attack review queues (default: result/AttackRuns)",
    )
...
def _run_recon(args: argparse.Namespace, *, prepare_attack: bool = False) -> int:
    ...
    if args.execute and (
        ...
        for target in selected_targets:
            ...
            if target.asset_type is AssetType.WILDCARD and key not in start_urls:
                value = _auto_wildcard_start_url(target, scope_document) if args.auto_wildcard_start else None
                if value:
                    print(f"{target.asset}: 자동 선택된 시작 URL > {value}")
                else:
                    value = input(f"{target.asset}: 로그인할 실제 타깃 시작 URL > ").strip()
                from aidast.recon.policy import validate_start_url_for_target
                validate_start_url_for_target(
                    value, asset_type=target.asset_type, asset=target.asset
                )
                start_urls[key] = value
        print(
            "로그인은 Endpoint Discovery에서 같은 Chromium 프로필로 진행합니다. "
            "로그인 중에는 프록시와 Scope 검사를 적용하지 않습니다."
        )
