...
class ReconToolError(RuntimeError):
    """A required Recon tool failed, distinct from a successful empty result."""


def _run_tool(
    command: list[str], *, tool_name: str, input_text: str | None = None,
    timeout: int = 120, required: bool = False,
) -> list[str]:
    if shutil.which(command[0]) is None:
        print(f"  [건너뜀] {tool_name}이 설치돼 있지 않음")
        if required:
            raise ReconToolError(f"{tool_name}이 설치돼 있지 않아 결과를 확인할 수 없음")
        return []
    try:
        ...
        if required:
            raise ReconToolError(f"{tool_name} 실행 실패: {exc}") from exc
        return []
    if completed.returncode != 0 and required:
        ...
        raise ReconToolError(
            f"{tool_name} 비정상 종료 (exit={completed.returncode}){suffix}"
        )
...
def run_subfinder(domain: str) -> list[str]:
    return _run_tool(
        ...
        tool_name="subfinder", required=True,
    )
