...
def tag_pending_observations(conn, *, scan_id: str, agent, batch_size: int = 200,
    ...
    rows = conn.execute('''
        SELECT o.observation_id, o.source_tool, o.discovery_kind, o.observed_url,
               o.association_method, o.observed_at, e.method, e.path,
    ...
    for offset in range(0, len(rows), batch_size):
        ...
        for row in rows[offset:offset + batch_size]:
            ...
            payload.append({
                ...
                'discovery_kind': row[2], 'url': row[3], 'method': row[6],
                'path': row[7], 'observed_at': row[5], 'evidence': evidence,
                'phase': row[2],
                'page_url': row[8], 'page_title': row[9],
                'action': row[11], 'auth_state': row[12],
