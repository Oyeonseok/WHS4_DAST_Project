...
승인된 버그바운티 Scope를 정책으로 변환하고 AI-Dast Recon과 태깅 뒤에
Native Attack, Chaining, Shared Validation, case 기반 Report를 연결하는
AI DAST CLI입니다. 전체 단계 순서, 실행 gate, DB 상태 전이와 재개는 결정론적인
Python 오케스트레이터가 관리합니다. 로그인된 Codex CLI는 각 단계에서 제한된
구조화 판단과 Skill 실행을 담당하며, 전체 파이프라인을 자율적으로 지휘하는
상위 Main Agent는 없습니다. 실제 네트워크 요청은 Python 검증기와 정책 경계를
통과한 경우에만 실행합니다.
...
`katana`, `ffuf`, `mitmdump`가 필요합니다. 일부 선택 도구는 설치되지 않으면
건너뛰지만, Wildcard 자산 발견에 필요한 Subfinder가 없거나 실패하면 이를 빈 결과로
취급하지 않고 해당 타깃을 실패 처리합니다. 정책 강제에 필요한 `mitmdump`를 시작하지
못하면 실행을 중단합니다.
...
├── Surface.json
├── ReconReview.json
├── Runs/<scan_id>/Recon.db, Surface.json, Handoff.json
├── AttackRuns/<scan_id>/Pipeline.db
├── AttackRun/                     # legacy Attack.db
├── ValidationRun/                 # legacy Validation.db
├── ReportRun/                     # case 또는 legacy Report.db
└── .aidast_sessions/
```
...
3. Codex 기반 planning adapter가 승인된 `Scope.md`를 읽고 구조화된 Recon Plan을 생성합니다.
...
`--all-targets`는 계획 모델이 큰 Scope 일부를 누락하더라도 선택된 실행 가능
canonical 타깃을 모두 Recon Plan에 포함하고, exact web 타깃은 DNS·HTTP Probe·Origin·
Endpoint Discovery를 수행합니다. Wildcard는 로그인 시작 URL로 정책을 좁히지 않고
먼저 승인된 wildcard 범위에서 자산 발견을 수행하며, 발견된 구체 호스트의 Endpoint
Discovery에서 필요하면 로그인합니다. `--all-targets`에서는 wildcard 자산 발견을
모든 명시 타깃의 조사보다 먼저 실행하고, 발견된 호스트의 DNS부터 Endpoint
Discovery까지도 기존 명시 타깃보다 먼저 처리합니다. 일반 실행 모드의 큐 순서는
변경하지 않습니다.

Wildcard 자산 발견 후보는 승인 Scope별 `result/Scope/<platform>/<program>/AssetDiscovery.db`
에 보존됩니다. 기본적으로 wildcard당 한 실행에서 최대 25개 호스트를 처리하며,
나머지 후보와 중단된 호스트는 다음 실행에서 이어 처리합니다. `--asset-discovery-batch-size`
로 회차당 처리량을 조절할 수 있습니다. 후보를 버리는 제한이 아니라 실행을 나누는
배치이며, 정책 밖 결과와 이미 명시적으로 선택한 타깃은 실행 후보에서 제외됩니다.
타깃별 실패는 해당 작업의 의존 작업만 건너뛰고 독립 타깃은 계속 진행합니다. 실패가
있어도 수집된 결과와 Surface를 저장하며, 스캔 상태는 `completed_with_errors`로 표시하고
CLI 종료 코드는 2를 반환합니다.
...
`--max-rps`와 `--max-requests`는 Codex 정책 생성 결과보다 낮은 상한만 적용합니다.
...
세션 유효성 검사는 전체 타깃 사전 검사가 아니라 실제 해당 타깃의 HTTP 단계에서
수행됩니다. 한 타깃의 세션 오류는 해당 타깃의 후속 단계만 건너뛰고 다른 타깃은 계속
진행합니다. 단, system-browser의 로그인 세션 수집 자체가 취소되거나 실패하면 실행을
시작할 수 없으므로 수집 단계에서 중단됩니다.
...
mitmproxy는 브라우저 동일-origin API(1), HTML document/redirect(2), 기타 API·폼
요청(3), Katana(4), ffuf(5), 정적·중복 요청(6)으로 분류해 예산을 적용합니다.
브라우저 API/document를 위한 20% 예약분은 저우선순위 트래픽이 먼저 소진하지
못합니다. 이는 이미 발생 중인 네트워크 요청을 모아 재정렬하는 큐가 아니라,
도구 출처 표식과 브라우저 요청 문맥을 이용한 프록시 admission control입니다.
실행 순서는 현재 인증된 브라우저/API 우선 관측 → Katana 경로 발견 → 새 Katana
경로의 Playwright 상호작용 확장 → ffuf입니다. Playwright의 두 pass는 방문 URL과
페이지 상한을 공유하며, ffuf 워드리스트 요청은 브라우저 관측이 끝난 뒤 실행합니다.
중복은 예산에서 제외되고, 정적 요청은 별도 건수로 집계되며, 예산 초과로 차단된
후보는 `deferred_candidates`에 저장됩니다. 차단된 요청은 예산 사용량에 더하지 않아
저우선순위 초과 시도가 브라우저 예약 예산까지 잠식하지 않습니다.

실행 직전에 Codex 정책 생성 호출이 승인된 `Scope.md`를 구조화된 `TargetPolicy.json`으로
...
저장 결과를 read-only로 검토한 뒤 태깅을 완료합니다. 이후 Python 오케스트레이터가
Handoff 검증, `Pipeline.db` 생성, Native Attack, Chaining, Shared Validation을
정해진 순서로 실행합니다. `CodexMainAgent`는 단계별 Codex 호출 adapter 이름이며,
전체 실행 순서나 stage gate를 결정하는 상위 Agent가 아닙니다.
...
`Pipeline.db`를 만든 뒤, Attack/Chaining/Validation용 schema는 복제본에만 추가합니다.
복제 전후 원본 해시가 달라지면 파이프라인을 중단합니다.

통합 `aidast run`의 종료 지점은 Shared Validation입니다. Report는 Validation 결과의
case를 검토해 별도 명령으로 생성합니다.

```text
Scope → AI-Dast Recon → 태깅 → Handoff → Pipeline.db
      → Native Attack → Chaining → Shared Validation

Shared Validation case → case 기반 Report
```

### Legacy Attack 경로

다음 명령은 persisted-data 호환을 위해 남겨 둔 기존 오프라인 `Attack.db` 경로입니다.
통합 `aidast run`에서 사용하는 Native Attack 경로와 구분해야 합니다.
...
따라서 이 legacy Attack 단계는 SQL injection, XSS, IDOR 같은 취약점을 능동적으로
...
기본 Validation은 별도 `Validation.db`를 만들지 않고 Native Attack과 Chaining이 사용한
동일한 `Pipeline.db`에 case, attempt, evidence와 decision을 기록합니다. Chaining stage가
`completed` 또는 `skipped`인 경우에만 시작할 수 있습니다.

```bash
aidast validate run result/AttackRuns/<scan_id>/Pipeline.db \
  --scan-id <scan_id> \
  --policy result/Runs/<scan_id>/TargetPolicy.json
...
aidast validate resume result/AttackRuns/<scan_id>/Pipeline.db \
  --stage-run-id <stage_run_id> \
  --policy result/Runs/<scan_id>/TargetPolicy.json

aidast validate status result/AttackRuns/<scan_id>/Pipeline.db \
  --scan-id <scan_id>
```

Validation은 후보 무결성과 현재 정책을 다시 검사하고 HTTP, browser, OOB, chain
reproduction adapter 뒤에서 positive/negative control과 반복 관측을 수행합니다.
최종 상태는 `CONFIRMED`, `DISPROVEN`, `OUT_OF_SCOPE`, `KNOWN`, `UNDERPOWERED`,
`BLOCKED`, `INCONCLUSIVE`, `CONTESTED` 중 하나이며 모델이 직접 최종 상태를
지정하지 않습니다. 중단된 case는 같은 stage run과 audit history를 유지해 재개합니다.

### Case 기반 Report

Report는 `Pipeline.db`에서 하나의 Validation `case_id`를 선택해 로컬 초안을 만듭니다.
`CONFIRMED`만 draft 대상이고, `KNOWN`은 원본 case를 가리키며 `CONTESTED`는
review-only로 남습니다. 지원 플랫폼은 HackerOne, Intigriti, Bugcrowd이며 자동 제출은
하지 않습니다.

```bash
aidast report run result/AttackRuns/<scan_id>/Pipeline.db \
  --case-id <case_id> \
  --platform hackerone \
  --output-dir result/ReportRun/<scan_id>

aidast report status result/ReportRun/<scan_id>/Report.db
```

결과의 `Report.db`, `Report.json`, `Report.md`는 Validation decision과 인용 evidence에
묶입니다. 해당 decision hash가 바뀌면 기존 Report는 stale로 판정됩니다.

### Legacy Validation/Report 호환

기존 `Attack.db → Validation.db → Report.db` 데이터는 삭제하거나 자동 변환하지 않습니다.
`--run-id`와 `--validation-id`를 명시한 기존 명령은 persisted-data 호환 경로로
계속 지원합니다.

```bash
aidast validate run result/AttackRun/Attack.db \
  --run-id <run_id> \
  --finding-id <finding_id> \
  --output-dir result/ValidationRun/<scan_id>

aidast report run result/ValidationRun/<scan_id>/Validation.db \
...
핵심 단계는 `src/aidast/recon`, `pipeline`, `attack`, `chaining`, `validation`,
`reporting`으로 분리되어 있습니다. Python stage orchestration은
`src/aidast/orchestration`, 제한된 Codex 호출 adapter는 `src/aidast/agents`,
각 단계의 로컬 Skill은 `src/aidast/skills` 아래에 둡니다. 기존 오프라인
Validation 구현은 `src/aidast/validation/legacy`, 기존 Report 구현은 reporting의
legacy 모듈로 보존합니다.

설계와 변경 이력은 `docs/design`, `docs/changes`, 외부 출처 보존 자료는
`docs/third-party`, 공용 wordlist는 `resources/wordlists`, 팀 전달용 ZIP은 `dist`에
둡니다. Scope, 세션, DB, JSON 등 실행 중 생성되는 자료는 모두 `result/` 아래에
두며 Git과 배포 ZIP에 포함하지 않습니다.
