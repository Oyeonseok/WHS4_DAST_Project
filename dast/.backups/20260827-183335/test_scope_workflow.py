...
from aidast.agents.main import CodexMainAgent, GroundingError, MainAgentError
...
class ScopeCoordinatorTests(unittest.TestCase):
    ...
    def test_incomplete_capture_writes_debug_dump_without_creating_scope_dir(
        self,
    ) -> None:
        class PartialMainAgent:
            def collect_scope(
        ...
        with tempfile.TemporaryDirectory() as temporary_dir:
            output = Path(temporary_dir) / "Scope"
            coordinator = ScopeCoordinator(output)
            with self.assertRaisesRegex(CoordinatorError, "debug capture saved to"):
                coordinator.collect(
                    "https://bugcrowd.com/example",
                    main_agent=PartialMainAgent(),
                    approved_by="reviewer",
            ...
            self.assertFalse(output.exists())
            debug_dir = output.parent / ".debug" / output.name
            dumps = list(debug_dir.glob("capture-*.json"))
            self.assertEqual(len(dumps), 1)
            payload = json.loads(dumps[0].read_text(encoding="utf-8"))
            self.assertEqual(payload["capture_status"], "PARTIAL")
            self.assertEqual(payload["capture_reason"], "CONTENT_INCOMPLETE")
            self.assertIn("*.example.com is in scope", payload["captured_text"])
            self.assertEqual(
                payload["analysis"]["program_name"], "Example <Program>"
            )
    ...
    def test_partial_native_capture_still_adopts_richer_partial_fallback(
        self,
    ) -> None:
        # Regression test: a native capture that is PARTIAL (not BLOCKED)
        # with JAVASCRIPT_RENDER_INCOMPLETE must still adopt the fallback's
        # page, even when the fallback itself does not reach COMPLETE.
        # Previously the fallback result was silently discarded whenever the
        # native capture wasn't BLOCKED and the fallback wasn't COMPLETE,
        # even though the fallback ran a real browser and captured far more
        # text than the native (near-empty) capture.
        richer_text = (
            "Example public bug bounty policy with explicit program rules. "
        ) * 20

        class TinyNativeMainAgent:
            def collect_scope(
                self, program_url: str
            ) -> tuple[ProgramPage, ScopeAnalysis]:
                return (
                    sample_page().model_copy(
                        update={
                            "capture_status": CaptureStatus.PARTIAL,
                            "capture_reason": CaptureReason.JAVASCRIPT_RENDER_INCOMPLETE,
                            "text": "Register\nLogin",
        ...
        class StillPartialFallbackReader:
            def read(self, url: str) -> ProgramPage:
        ...
        with tempfile.TemporaryDirectory() as temporary_dir:
            ...
            with self.assertRaisesRegex(CoordinatorError, "debug capture saved to"):
                ScopeCoordinator(output).collect(
                    "https://bugcrowd.com/example",
                    main_agent=TinyNativeMainAgent(),
                    fallback_reader=StillPartialFallbackReader(),
                    approved_by="reviewer",
                    review=lambda _: True,
                )
            debug_dir = output.parent / ".debug" / output.name
            ...
            payload = json.loads(dumps[0].read_text(encoding="utf-8"))
            self.assertEqual(payload["capture_reason"], "CONTENT_INCOMPLETE")
            self.assertEqual(payload["captured_text"], richer_text)

    def test_grounding_failure_writes_debug_dump_without_creating_scope_dir(
        self,
    ) -> None:
        class UngroundedMainAgent:
            def collect_scope(
                self, program_url: str
            ) -> tuple[ProgramPage, ScopeAnalysis]:
                page = sample_page()
                analysis = sample_analysis().model_copy(
                    update={
                        "source_evidence": [
                            SourceEvidence(section="Testing", quote="Testing")
                        ]
                    }
                )
                raise GroundingError(
                    "Codex returned an ungrounded source quote: Testing",
                    page=page,
                    analysis=analysis,
                )
        ...
        with tempfile.TemporaryDirectory() as temporary_dir:
            ...
            with self.assertRaisesRegex(CoordinatorError, "debug capture saved to"):
                ScopeCoordinator(output).collect(
                    "https://bugcrowd.com/example",
                    main_agent=UngroundedMainAgent(),
                    approved_by="reviewer",
                    review=lambda _: True,
                )
            self.assertFalse(output.exists())
            ...
            payload = json.loads(dumps[0].read_text(encoding="utf-8"))
            self.assertEqual(
                payload["analysis"]["source_evidence"],
                [{"section": "Testing", "quote": "Testing"}],
            )
...
class ScopePathTests(unittest.TestCase):
    def test_identifies_supported_program_paths(self) -> None:
        cases = {
            ...
            "https://app.intigriti.com/researcher/programs/adobe/adobepublic": (
                "intigriti",
                "adobe-adobepublic",
            ),
        }
