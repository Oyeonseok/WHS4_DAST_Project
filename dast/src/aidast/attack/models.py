...
class AttackFinding(BaseModel):
    ...
    endpoint_id: str | None = Field(
        default=None, description="recon DB의 endpoint_id. 없으면 null"
    )
    vuln_type: str = "IDOR"
    title: str
    description: str
...
class GateDetail(BaseModel):
    ...
    passed: bool | None = Field(
        description="true=pass, false=fail, null=N/A"
    )
    detail: str = ""
...
class ValidationResult(BaseModel):
    ...
    gate_results: dict[str, GateDetail] = Field(default_factory=dict)
    reasoning: str = ""
